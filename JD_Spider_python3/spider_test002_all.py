...
class spiders:
    ...
    def get_pids(self):
        ...
        for l in elements:
            data_pid = l.get_attribute('data-sku')
            print(data_pid)
            if (data_pid):
    ...
    def get_product_comments(self):
        ...
        for i in self.pids:
            pageno = 0
            product_id = i
            product_url = "https://sclub.jd.com/comment/productPageComments.action?productId=" + product_id + "&score=0&sortType=5&page=0&pageSize=10"
            ...
            try:
                while pageno <= comment_page_num:
                    ...
                    for comment in bjson['comments']:
                        ...
                        self.sql.save_product_comment(product_id, user_id, user_name, comment_content, referenceName,
                                                      creationTime)
                    pageno += 1
            except:
                print("解析发生错误...")
